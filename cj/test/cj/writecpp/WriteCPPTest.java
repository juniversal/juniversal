...
public class WriteCPPTest {
	...
	@Test public void variableDeclarationTest() {
		...
		try {
			...
			assertEquals(
					"cj.SourceNotSupportedException: <unknown-file> (line 2, col 1): long type isn't supported by default; need to specify target C++ type for 64 bit int",
					e.toString());
		}
			...
			int sourceTabStop, String expectedCPPOutput)
	{
		...
		WriteCPPContext context = new WriteCPPContext((CompilationUnit) compilationUnit.getRoot(),
	...
	static WriteCPP m_writeCPP = null; 
	WriteCPP getWriteCPP() {
		if (m_writeCPP == null)
			m_writeCPP = new WriteCPP();
		return m_writeCPP;
	}
