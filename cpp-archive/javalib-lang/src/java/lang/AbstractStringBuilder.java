 ...
 * Strings. This class is intended as a base class for {@link StringBuffer} and
 * {@link StringBuilder}.
 * 
 * JUniversal REMOVED:
 * 
...
abstract class AbstractStringBuilder {
    ...
    private void enlargeBuffer(int min) {
        ...
        JUniversalHelper.arraycopy(value, newData, count);
        value = newData;
        shared = false;
    }
    ...
    final void append0(char[] chars) {
        ...
        JUniversalHelper.arraycopy(chars, 0, value, count, chars.length);
        count = newSize;
    }
    ...
    final void append0(char[] chars, int offset, int length) {
        ...
        if (offset > chars.length || offset < 0) {
			...
			throw new java.lang.ArrayIndexOutOfBoundsException(new StringBuilder(
					"Offset out of bounds : ").append(offset).toString());
        }
        if (length < 0 || chars.length - offset < length) {
            // luni.18=Length out of bounds \: {0}
            throw new java.lang.ArrayIndexOutOfBoundsException(
            		new StringBuilder("Length out of bounds : ").append(length).toString());
        }
        ...
        JUniversalHelper.arraycopy(chars, offset, value, count, length);
        count = newSize;
    }
    ...
    final void delete0(int start, int end) {
        if (start >= 0) {
            ...
            if (end > start) {
                ...
                if (length >= 0) {
                    if (!shared) {
                        JUniversalHelper.arraycopy(value, end, value, start, length);
                    } else {
                        char[] newData = new char[value.length];
                        JUniversalHelper.arraycopy(value, 0, newData, 0, start);
                        JUniversalHelper.arraycopy(value, end, newData, start, length);
                        value = newData;
                        shared = false;
                    }
                }
    ...
    final void deleteCharAt0(int location) {
        if (0 > location || location >= count) {
            throw new java.lang.StringIndexOutOfBoundsException(location);
        }
        int length = count - location - 1;
        if (length > 0) {
            if (!shared) {
                JUniversalHelper.arraycopy(value, location + 1, value, location, length);
            } else {
                char[] newData = new char[value.length];
                JUniversalHelper.arraycopy(value, newData, location);
				JUniversalHelper.arraycopy(value, location + 1, newData, location, length);
                value = newData;
                shared = false;
            }
        }
    ...
    public void getChars(int start, int end, char[] dest, int destStart) {
        ...
        JUniversalHelper.arraycopy(value, start, dest, destStart, end - start);
    }
    ...
    final void insert0(int index, char[] chars) {
        ...
        if (chars.length != 0) {
            ...
            JUniversalHelper.arraycopy(chars, 0, value, index, chars.length);
            count += chars.length;
        }
    }
    ...
    final void insert0(int index, char[] chars, int start, int length) {
        if (0 <= index && index <= count) {
            ...
            if (start >= 0 && 0 <= length && length <= chars.length - start) {
                if (length != 0) {
                    move(length, index);
                    JUniversalHelper.arraycopy(chars, start, value, index, length);
                    count += length;
                }
                return;
            }
            throw new java.lang.StringIndexOutOfBoundsException("offset " + start //$NON-NLS-1$
    ...
    private void move(int size, int index) {
        ...
        if (value.length - count >= size) {
            if (!shared) {
                JUniversalHelper.arraycopy(value, index, value, index + size, count
        ...
        JUniversalHelper.arraycopy(value, 0, newData, 0, index);
        // index == count case is no-op
        JUniversalHelper.arraycopy(value, index, newData, index + size, count - index);
        value = newData;
        shared = false;
    }
        ...
        if (start >= 0) {
            ...
            if (end > start) {
                ...
                if (diff > 0) { // replacing with fewer characters
                    if (!shared) {
                        // index == count case is no-op
                        JUniversalHelper.arraycopy(value, end, value, start
                                + stringLength, count - end);
                    } else {
                        char[] newData = new char[value.length];
                        JUniversalHelper.arraycopy(value, 0, newData, 0, start);
                        // index == count case is no-op
                        JUniversalHelper.arraycopy(value, end, newData, start
                                + stringLength, count - end);
                        value = newData;
                        shared = false;
                    }
     ...
     * 
     * JUniversal CHANGE: Made so always creates a new string, that's never shared
     * 
    ...
    public void trimToSize() {
        if (count < value.length) {
            ...
            JUniversalHelper.arraycopy(value, newValue, count);
            value = newValue;
            shared = false;
        }
    }
