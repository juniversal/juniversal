...
public class JavaToObjectiveC {
    ...
    public void convert(String taskName, CommonFileSet sourcepath, CommonFileSet classpath, CommonFileSet sourceFilesToConvert,
        ...
        CommonFileSet j2objcClasspath = new CommonFileSet(classpath);
        j2objcClasspath.add(new File(j2objcLibDirectory, "j2objc_annotations.jar"));

        CommonJavaExec javaExec = project.createJavaExec(taskName, "Convert Java to Objective C",
                new File(j2objcLibDirectory, "j2objc.jar"));
        ...
        for (File file : sourceFilesToConvert.getFiles()) {
            javaExec.addArg(file.getAbsolutePath());
        }

        project.info(javaExec.getCommandLine());
        javaExec.exec();
    }
